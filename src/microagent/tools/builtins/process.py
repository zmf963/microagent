...
async def process(
    ...
    command: Annotated[str | None, Field(description="Shell command to run (for action=start)")] = None,
    session_id: Annotated[str | None, Field(description="Process ID from start")] = None,
    data: Annotated[str | None, Field(description="Data to write to stdin (for action=write)")] = None,
    ...
    match action:
        ...
        case _:
            return ToolResult.error(f"unknown action: {action}. Valid: start, poll, log, kill, wait, list, write")
